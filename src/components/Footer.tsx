                    ...
                    <div className="flex items-center gap-4">
                        {profile.socials.map((s) => (
                            <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                                className="text-slate-400 hover:text-accent transition-colors" aria-label={s.label}>
                                <SmallIcon icon={s.icon} />
                            </a>
                        ))}
...
function SmallIcon({ icon }: { icon: string }) {
    ...
    if (icon === "email") return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="4" width="20" height="16" rx="2" /><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" /></svg>;
    return null;
}
